        ...
        if provider == "gemini":
            ...
            for model_name in models_to_try:
                try:
                    ...
                    resp = requests.post(
                        url, params={"key": gemini_key}, json=payload, timeout=20
                    )
                    ...
                    if resp.ok:
                        ...
                        try:
                            ...
                            last_error = (
                                f"{model_name}: HTTP {resp.status_code} - {error_msg}"
                            )
                        except:
...
def main():
    ...
    with tabs[0]:
        ...
        with st.expander(
            f"ℹ️ Why {LLM_PROVIDERS[selected_provider]['name']}?", expanded=False
        ):
        ...
        with col1:
            language = st.selectbox(
                "Fact Check Language", options=lang_options, index=0
            )
        with col2:
        ...
        if run:
            if not (text or title or url):
                ...
                with st.spinner(
                    f"🔄 Analyzing with {LLM_PROVIDERS[selected_provider]['name']}..."
                ):
        ...
        if st.session_state.analysis_results:
            ...
            if results.get("translated"):
                st.info(
                    f"✅ Text was translated from {results['language'].upper()} to English for analysis"
                )
            ...
            if results["google_items"]:
                for it in results["google_items"]:
                    ...
                    st.markdown(
                        f"{rating_icon} **{rating}** — {it.get('publisher')} — [View Source]({it.get('url')})"
                    )
            else:
            ...
            # LLM Explanation
            st.markdown(
                f"### 🤖 Analysis by {LLM_PROVIDERS[results['provider']]['name']}"
            )
    ...
    with tabs[1]:
        ...
        if not st.session_state.analysis_results:
            ...
            # Chat input
            if user_input := st.chat_input("Your argument or question:"):
            ...
            with col1:
                if st.button("Why is this fake/true?"):
                    st.session_state.chat_history.append(
                        {
                            "role": "user",
                            "content": "Explain in detail why you think this claim is fake or true.",
                        }
                    )
                    st.rerun()
            with col2:
                if st.button("Show me evidence"):
                    st.session_state.chat_history.append(
                        {
                            "role": "user",
                            "content": "What specific evidence supports or contradicts this claim?",
                        }
                    )
                    st.rerun()
            with col3:
                if st.button("I disagree"):
                    st.session_state.chat_history.append(
                        {
                            "role": "user",
                            "content": "I disagree with your assessment. Can you consider alternative perspectives?",
                        }
                    )
                    st.rerun()
    ...
    with tabs[2]:
        ...
        if not st.session_state.analysis_results:
            ...
            st.markdown(
                f"**Original Analysis:** {LLM_PROVIDERS[results['provider']]['name']}"
            )
            ...
            if st.button("🔄 Run Comparison", type="primary"):
                with st.spinner(
                    f"Running analysis with {LLM_PROVIDERS[second_provider]['name']}..."
                ):
    ...
    with tabs[3]:
        ...
        if not st.session_state.analysis_results:
            ...
            if results["google_items"]:
                st.markdown(
                    f"**Found {len(results['google_items'])} external fact-checks**"
                )
                ...
                for idx, item in enumerate(results["google_items"], 1):
                    with st.expander(
                        f"Source {idx}: {item.get('publisher', 'Unknown')}"
                    ):
                        st.markdown(
                            f"**Rating:** {item.get('textual_rating', 'Unknown')}"
                        )
                        ...
                        st.markdown(
                            f"**Review Date:** {item.get('review_date', 'Unknown')}"
                        )
            else:
                st.warning(
                    "No external sources found. This claim may be too recent or too specific."
                )
