...
def get_shap_explainer():
    ...
    if not os.path.exists(model_path):
        print(f"Local model not found at {model_path}. Please ensure the model is downloaded.")
        return None
    ...
    def f(x):
        tv = torch.tensor([tokenizer.encode(v, padding='max_length', max_length=MODEL_MAX_LENGTH, truncation=True) for v in x]).cuda() if torch.cuda.is_available() else torch.tensor([tokenizer.encode(v, padding='max_length', max_length=MODEL_MAX_LENGTH, truncation=True) for v in x])
    ...
    pipe = transformers.pipeline('text-classification', model=model, tokenizer=tokenizer, top_k=None, device=0 if torch.cuda.is_available() else -1)
